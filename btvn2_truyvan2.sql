...
1.	Tìm tất cả các sách thuộc thể loại “Văn học”
SELECT *
FROM Books
WHERE Genre = 'Văn học'
...
SELECT bk.Title
FROM Books bk
LEFT JOIN Borrowings b ON bk.BookID = b.BookID
...
SUM(DATEDIFF(IFNULL(b.ReturnDate, CURDATE()), b.BorrowDate)) AS TongNgay
FROM Members m
JOIN Borrowings b ON m.MemberID = b.MemberID
...
SELECT m.Name,
COUNT(b.BorrowID) AS SoLanMuonSach,
SUM(IF(b.ReturnDate IS NULL, 1, 0)) AS SoSachChuaTra
FROM Members m
...
WHERE bk.Title = 'Tiểu thuyết'
GROUP BY m.Name
HAVING (SoLanMuonSach > 2)
